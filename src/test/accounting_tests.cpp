...
GetResults(CWalletDB& walletdb, std::map<int64, CAccountingEntry>& results)
{
    ...
    results.clear();
    BOOST_CHECK(walletdb.ReorderTransactions(pwalletTest) == DB_LOAD_OK);
...
{
    CWalletDB walletdb(pwalletTest->strWalletFile);
    ...
    pwalletTest->AddToWallet(wtx);
    vpwtx.push_back(&pwalletTest->mapWallet[wtx.GetHash()]);
    ...
    BOOST_CHECK(pwalletTest->nOrderPosNext == 3);
    ...
    ae.nOrderPos = pwalletTest->IncOrderPosNext();
    walletdb.WriteAccountingEntry(ae);

    GetResults(walletdb, results);

    BOOST_CHECK(results.size() == 3);
    BOOST_CHECK(pwalletTest->nOrderPosNext == 4);
    BOOST_CHECK(results[0].nTime == 1333333333);
    BOOST_CHECK(1 == vpwtx[0]->nOrderPos);
    BOOST_CHECK(results[2].nTime == 1333333336);
    ...
    BOOST_CHECK(results.size() == 3);
    BOOST_CHECK(pwalletTest->nOrderPosNext == 6);
    ...
    BOOST_CHECK(pwalletTest->nOrderPosNext == 7);
    BOOST_CHECK(0 == vpwtx[2]->nOrderPos);
    BOOST_CHECK(results[1].nTime == 1333333333);
    BOOST_CHECK(2 == vpwtx[0]->nOrderPos);
    BOOST_CHECK(results[3].nTime == 1333333336);
