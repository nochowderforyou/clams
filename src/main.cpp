...
{
    return ComputeMaxBits(bnProofOfStakeLimitEasier, nBase, nTime);
}
...
{
    CBigNum bnTargetLimit = fProofOfStake ? bnProofOfStakeLimitEasier : bnProofOfWorkLimit;
    int64_t currentTargetSpacing = nTargetStakeSpacing;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    else if (pindexLast->nHeight < LOWER_DIFFICULTY_START)
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfStake())
    {
        ...
        if (!CheckProofOfStake(vtx[1], nBits, nHeight, hashProof, targetProofOfStake))
        {
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Clam", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        ...
        nStakeMinAge = 1 * 60; // test net min age is 1 hour
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1397512438;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 2054231 : 1279100;
 
 
 
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xef10b32afd53e4a6ebb8bdb0486c6acbe9b43afe3dfa538e913b89bb1319ff96"));
        block.print();
