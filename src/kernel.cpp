...
//
bool CheckStakeKernelHash(unsigned int nBits, int nHeight, const CBlock& blockFrom, unsigned int nTxPrevOffset, const CTransaction& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, uint256& targetProofOfStake, bool fPrintProofOfStake)
{
    ...
    if (nHeight >= TXID_IN_STAKE_HASH_START)
        ss << txPrev.GetHash();
...
bool CheckProofOfStake(const CTransaction& tx, unsigned int nBits, int nHeight, uint256& hashProofOfStake, uint256& targetProofOfStake)
{
    ...
    if (!CheckStakeKernelHash(nBits, nHeight, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, txPrev, txin.prevout, tx.nTime, hashProofOfStake, targetProofOfStake, fDebug))
